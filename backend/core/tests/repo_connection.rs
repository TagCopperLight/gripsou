...
async fn insert_pending_creates_row(pool: PgPool) -> anyhow::Result<()> {
    ...
    sqlx::query(
        "insert into users (id, email, name, password_hash) values ($1, $2, 'T', 'x')",
    )
    .bind(user_id)
    ...
    let id = gripsou_core::repo::connection::insert_pending(
        &pool, user_id, "powens", "My bank",
    )
    .await?;

    let (status,): (String,) =
        sqlx::query_as("select status from connection where id=$1")
            .bind(id)
            .fetch_one(&pool)
            .await?;
...
async fn finish_connect_updates_credentials_and_status(pool: PgPool) -> anyhow::Result<()> {
    ...
    let updated = gripsou_core::repo::connection::finish_connect(&pool, id, user_id, creds.clone()).await?;
    assert!(updated);
    ...
    let not_updated = gripsou_core::repo::connection::finish_connect(
        &pool, id, uuid::Uuid::new_v4(), serde_json::json!({}),
    ).await?;
    assert!(!not_updated);
    Ok(())
}
...
async fn delete_connection_cascades(pool: PgPool) -> anyhow::Result<()> {
    ...
    let deleted =
        gripsou_core::repo::connection::delete_connection(&pool, user_id, conn).await?;
    assert!(deleted);
