...
use common::{
    cash_holding, checking_account, deposit_txn, equity_holding, seed_connection,
};
...
async fn ingest_then_reingest_is_idempotent(pool: PgPool) -> anyhow::Result<()> {
    ...
    let holdings: i64 = sqlx::query_scalar("select count(*) from holding").fetch_one(&pool).await?;
    let txns: i64 = sqlx::query_scalar("select count(*) from transaction").fetch_one(&pool).await?;
    let snaps: i64 = sqlx::query_scalar("select count(*) from holding_snapshot").fetch_one(&pool).await?;
    assert_eq!((accounts, holdings, txns, snaps), (1, 2, 1, 2), "no duplicates after re-ingest");
    Ok(())
}
...
async fn cash_instrument_is_shared_across_accounts(pool: PgPool) -> anyhow::Result<()> {
    ...
    let cash_instruments: i64 =
        sqlx::query_scalar("select count(*) from instrument where kind = 'cash'")
            .fetch_one(&pool)
            .await?;
    assert_eq!(cash_instruments, 1, "one EUR cash instrument shared by both accounts");
    Ok(())
}
...
async fn snapshot_value_matrix(pool: PgPool) -> anyhow::Result<()> {
    ...
    let sync = SyncResult {
        ...
        holdings: vec![
            ...
            equity_holding("acct-1", "US0378331005", Decimal::new(3, 0), Decimal::new(450, 0), Some(Decimal::new(600, 0))), // valued -> 600
            equity_holding("acct-1", "US5949181045", Decimal::new(2, 0), Decimal::new(300, 0), None), // unpriced -> 0
        ],
        transactions: vec![],
    };
    ingest(&pool, conn_id, &sync).await?;
    ...
    let values: Vec<Decimal> =
        sqlx::query_scalar("select value from holding_snapshot order by value")
            .fetch_all(&pool)
            .await?;
    assert_eq!(values, vec![Decimal::new(0, 0), Decimal::new(100, 0), Decimal::new(600, 0)]);
    Ok(())
}
